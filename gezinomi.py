...
# İş Problemi:
# "Gezinomi yaptığı satışların bazı özelliklerini kullanarak seviye tabanlı(level based)
# yeni satış tanımları oluşturmak ve bu yeni satış tanımlarına göre
# segmentler oluşturup bu segmentlere göre yeni gelebilecek müşterilerin
...
#Görev1: miuul_gezinomi.xlsx dosyasını okutunuz ve veri seti ile ilgili genel bilgileri gösteriniz.

import pandas as pd
pd.set_option("display.max_columns", None)
df = pd.read_excel("C:\\Users\\busra\\OneDrive\\Masaüstü\\Gezinomi\\miuul_gezinomi.xlsx")
...
print(df.groupby(by=["ConceptName","SaleCityName"]).agg({"Price": "mean"}))
...
print(df.head(50).to_excel("eb_score.xlsx", index=False))
...
print(df.groupby(by=["SaleCityName","ConceptName","EB_Score"]).agg({"Price": "mean"}))
# Şehir-Concept-Sezon kırılımında ücret ortalamaları

print(df.groupby(by=["SaleCityName","ConceptName","Seasons"]).agg({"Price": "mean"}))

# Şehir-Concept-CInday kırılımında ücret ortalamaları

print(df.groupby(by=["SaleCityName","ConceptName","CInDay"]).agg({"Price": "mean"}))
